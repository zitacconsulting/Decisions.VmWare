...
[AutoRegisterStep("Disconnect CDROM", "Integration", "VmWare", "VM")]
[Writable]
public class DisconnectCDROM : BaseFlowAwareStep, ISyncStep, IDataConsumer, IDataProducer
{
    [WritableValue]
    ...
    public DataDescription[] InputData
    {
        get
        {
            ...
            dataDescriptionList.Add(new DataDescription((DecisionsType)new DecisionsNativeType(typeof(int)), "CDROM Key"));
    ...
    public ResultData Run(StepStartData data)
    {
        ...
        int? CDKey = data.Data["CDROM Key"] as int?;
        ...
        try
        {
            ...
            bool cdDriveFound = false;

            foreach (var device in vm.Config.Hardware.Device)
            {
                if (device.Key == CDKey && device is VirtualCdrom cdrom)
                {
                    cdDriveFound = true;

                    // Set the backing to indicate no media is present
                    cdrom.Backing = new VirtualCdromRemotePassthroughBackingInfo
                    {
                        DeviceName = "",
                        UseAutoDetect = false
                    };

                    // Set the connectable to false to disconnect it
                    ...
                    var deviceChange = new VirtualDeviceConfigSpec
                    {
                        Operation = VirtualDeviceConfigSpecOperation.edit,
                        Device = cdrom
                    };
                    configSpec.DeviceChange = new VirtualDeviceConfigSpec[] { deviceChange };
                    break;
                }
            }
            ...
            if (!cdDriveFound)
            {
                throw new Exception("Failed to add Find CDROM with Key:" + CDKey);
            }
            ...
            if (TaskResult.Info.State.ToString() == "error")
            {
                vimClient.Logout();
                vimClient.Disconnect();
                throw new Exception("Failed to disconnect CDROM:" + TaskResult.Info.Error.Fault.ToString() + " - " + TaskResult.Info.Error.LocalizedMessage.ToString());
            }
