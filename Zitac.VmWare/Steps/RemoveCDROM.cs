...
[AutoRegisterStep("Remove CDROM", "Integration", "VmWare", "VM")]
[Writable]
public class RemoveCDROM : BaseFlowAwareStep, ISyncStep, IDataConsumer, IDataProducer
{
    [WritableValue]
    ...
    public ResultData Run(StepStartData data)
    {
        ...
        try
        {
            ...
            if (TaskResult.Info.State.ToString() == "error")
            {
                vimClient.Logout();
                vimClient.Disconnect();
                throw new Exception("Failed to remove CDROM:" + TaskResult.Info.Error.Fault.ToString() + " - " + TaskResult.Info.Error.LocalizedMessage.ToString());
            }
